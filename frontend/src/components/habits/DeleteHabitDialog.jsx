...
const DeleteHabitDialog = ({ isOpen, habit, onClose, onDeleted }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await api.delete(`/habits/${habit.id}`);

      if (!response.data?.success) {
        throw new Error(response.data?.message || "Unable to delete habit.");
      }
              ...
              <div className="mt-5">
                ...
                <div className="mt-5 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-indigo-400/10 bg-indigo-500/10">
                      <FiTrash2 size={16} className="text-indigo-300" />
                    </div>

                    <div className="min-w-0">
                      <p className="habit-name-display truncate text-sm font-medium">
                        {habit.name}
                      </p>
                ...
                <div className="mt-4 rounded-xl border border-red-400/[0.08] bg-red-400/[0.035] px-4 py-3">
                  <p className="text-xs leading-5 text-red-300/80">
                    This will permanently remove the habit and its associated
